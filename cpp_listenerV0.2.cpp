...
public:
	...
	move_base_msgs::MoveBaseGoal goal_buf;
    ...
    nh.param("goal_offset", goal_offset, 0.5)
	...
	while (ros::ok())
	{
	    ...
	    if (people_flag)	{
			if (abs(goal_buf.target_pose.pose.position.x + goal_buf.target_pose.pose.position.y - 
			goal.target_pose.pose.orientation.x - goal.target_pose.pose.orientation.y )> goal_offset){
				goal_buf = goal;
				ac.sendGoal(goal);
			}
			else ROS_INFO("same goal!");
	    else {
		//pub.publish(" ");		
